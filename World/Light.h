...
struct Light {
  ...
  Light(Color color, Tuple position):intensity{color}, position{position}{};
  Light(std::array<float, 3> rgb, std::array<float, 3> p):intensity{Color(rgb[0], rgb[1], rgb[2])}, position{point(p[0], p[1], p[2])}{};
